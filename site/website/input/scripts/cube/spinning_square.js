...
const CubeFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_cube_free(ptr >>> 0, 1));

export class Cube {

    static __wrap(ptr) {
        ptr = ptr >>> 0;
        const obj = Object.create(Cube.prototype);
        obj.__wbg_ptr = ptr;
        CubeFinalization.register(obj, obj.__wbg_ptr, obj);
        return obj;
    }
     ...
     */
    static new() {
        const ret = wasm.cube_new();
        return Cube.__wrap(ret);
    }
    update() {
        wasm.cube_update(this.__wbg_ptr);
    }
    /**
    ...
    render(ctx, width, height, color) {
        ...
        wasm.cube_render(this.__wbg_ptr, ctx, width, height, ptr0, len0);
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('spinning_square_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
